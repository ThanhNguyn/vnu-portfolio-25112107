...
function Hero({ onViewCode }: { onViewCode: () => void }) {
  return (
          ...
          <Reveal delay={0.45} y={20}>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                onClick={onViewCode}
                className="inline-flex items-center gap-2 rounded-full border border-stone-900/30 px-5 py-3 text-sm text-stone-900 transition dark:border-white/15 dark:text-white/90 dark:hover:bg-white/10 cursor-pointer"
              >
                Mã nguồn Portfolio <Code size={14} />
              </motion.button>
            </div>
          </Reveal>
